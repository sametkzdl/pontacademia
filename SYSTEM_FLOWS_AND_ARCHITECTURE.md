...
| **4** | **Ders Tamamlama, Karşılıklı İşlendi Onayı & İşlenmedi/İtiraz Akışı** | Ders bitiminde taraflar onay/itiraz bildirir | `/student/lessons`<br>`/teacher/lessons`<br>`LessonService` | • **Çift Taraflı Tamamlama (`COMPLETED`)**: Hem öğretmen hem öğrenci 'Ders İşlendi' onayı verdiğinde statü `COMPLETED` olur.<br>• **Ders İşlenmedi / İtiraz / Ret (`REJECTED`)**: Taraflardan biri dersin işlendiğini onayladığında diğer taraf dersin yapılmadığını belirterek 'Ders İşlenmedi / İtiraz Et' gerekçeli reddi verebilir. Statü `REJECTED` olur, açıklama ve itiraz eden taraf `LessonLog` tarihçesine yazılarak Admin denetimine iletilir.<br>• **Gizli Değerlendirme**: Tamamlanan derste 1-5 yıldız ve yorum ile gizli geri bildirim bırakılır; bu yorumları **yalnızca Admin** okuyabilir. |
