...
graph TD
    ...
    F -->|Karşılıklı İşlendi Onayı| G[Ders Tamamlandı: COMPLETED]
    F -->|İşlenmedi / İtiraz / Ret Bildirimi| R[Ders Reddedildi / İptal: REJECTED]
...
| **4** | **Ders Tamamlama, Karşılıklı İşlendi Onayı & İşlenmedi (Admin Flag) Akışı** | Ders bitiminde taraflar onay/itiraz bildirir | `/student/lessons`<br>`/teacher/lessons`<br>`/admin/lessons`<br>`LessonService` | • **Çift Taraflı Tamamlama (`COMPLETED`)**: Hem öğretmen hem öğrenci 'Ders İşlendi' onayı verdiğinde statü `COMPLETED` olur.<br>• **Ders İşlenmedi Bildirimi (`ISLENMEDI`)**: Taraflardan biri dersin işlenmediğini bildirdiğinde veya itiraz ettiğinde statü `ISLENMEDI` olarak güncellenir.<br>• **Yalnızca Admin'e Özel Flag (`🚩`)**: Dersin işlenmediği bilgisi, bildiren taraf ve gerekçesi **yalnızca Admin Paneli**'nde kırmızı yönetici dikkat bayrağı (`🚩 YÖNETİCİ DİKKATİNE: DERSİN İŞLENMEDİĞİ BİLDİRİLDİ`) olarak listelenir; admin buradan onaylayabilir veya iptal edebilir. Öğrenci/öğretmen panellerinde ise sadece standart nötr statü görünür.<br>• **Gizli Değerlendirme**: Tamamlanan derste 1-5 yıldız ve yorum ile gizli geri bildirim bırakılır; bu yorumları **yalnızca Admin** okuyabilir. |
...
| **8** | **Profil Ders Ekleme & Bırakma Koruması** | Öğrenci profilinden ders yönetir | `/student/profile`<br>`/student/requests`<br>`ProfileService` | • **Yeni Ders Ekleme**: Öğrenci almak istediği yeni dersleri profilinden ekleyebilir; modalda var olan kayıtlı dersler kilitli (`🔒 Kayıtlı`) gösterilir ve mükerrer seçim engellenir.<br>• **Doğrudan Silme Yasağı**: Öğrenci aktif veya kayıtlı derslerini profilden doğrudan 'X' butonuna basarak silemez.<br>• **Ders Bırakma Talep Formu**: Kayıtlı bir dersi bırakmak/iptal etmek isteyen öğrenci, `/student/requests` üzerinden onaylı 'Ders Bırakma Talebi' oluşturmalıdır. Böylece aktif ders oturumları ve finansal planlar korunur. |
